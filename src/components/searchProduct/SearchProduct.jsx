...
const SearchProduct  =(prop)=>{
    const [dataFiler,setDataFilter]  = useState(prop.dataProduct.filter(x=>x.title.includes(prop.textSearch))||[])
    
    useEffect(()=>{
        console.log("filter seaarch",prop.dataProduct)
        let dataFilter = prop.dataProduct.filter(x=>x.title.includes(prop.textSearch))
            ...
            <ul className={`${dataFiler.length ===0 ? "" : " border "} basis-[580px] max-h-[400px] overflow-y-auto`} >
                            {
                                ...
                                <li key={product._id+"-"+index} className=' mt-1 min-w-[346px] h-[70px] pl-1 pr-1 border-b border-solid border-[#ddd] '>
